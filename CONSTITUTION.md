...
**Status:** draft v0.1 — written 2026-04-07. This document defines what engram is, what it is *not*, and the principles that should outlive any specific implementation. Everything below is meant to be negotiated; nothing below is meant to be ignored without a reason.

**Working name:** `engram`. Renaming is cheap right now and expensive later — decide before the first commit lands code.
...
engram unbundles these. **vstash is the substrate. engram is the loop.** They talk through vstash's stable Python API; engram never reaches into vstash internals.

If you wanted a one-line pitch:

> engram turns vstash from a *vector store you query* into *a memory you live with*.
...
4. **vstash is a hard dependency.** Do not reimplement retrieval. Do not reach into vstash internals. If vstash's public API is missing something engram needs, the right move is a vstash PR, not a workaround.
...
engram thinks of memory in three layers, borrowed from cognitive science but kept loose because the brain analogy is suggestive, not normative.
...
**engram's role:** decide whether the event is worth writing at all (`should_remember`), and provide convenience wrappers that resolve the right collection / project / tags from agent context.
...
**Storage:** also vstash, but with `layer="semantic"` and a different collection. The same retrieval pipeline, the same MMR dedup, the same RRF — engram does not reinvent any of it.

**engram's role:** the **consolidation pipeline**. Periodically (or on-demand) pull recent episodic chunks, ask an LLM to extract atomic facts, deduplicate against existing semantic memory, and write the survivors. This is the part that takes vstash from "search engine" to "agent that remembers."
...
**engram's role:** capture-on-success and recall-on-similar-task. This is the layer most agent frameworks ignore; we treat it as first-class.

These three layers share one substrate (vstash) and one query path (vstash's hybrid search). What differentiates them is:
- the *write policy* (when does engram decide to add a row to this layer?),
- the *recall policy* (when does engram pull from this layer into the agent's prompt?),
- and the *consolidation policy* (how does this layer feed the next?).

The agent loop calls engram for those policies. engram calls vstash for the actual storage and retrieval.
...
**The boundary is sacred.** engram does not import from `vstash._private`. It does not read SQLite tables directly. It does not bypass vstash's validation. If the boundary is in the way, the answer is to move the boundary in vstash, not to drill through it from above.
...
engram should be embarrassingly small at the top level. The smell test:
...
If something on this list ever lands in engram, it should be because we wrote down why and the document survived a re-read three weeks later.
...
- It runs against vstash + engram and against a baseline (no-engram, just vstash; or a competing framework if comparable).
...
1. **Name.** `engram` is a working title. Alternatives floated: `mneme`, `anamnesis`, `vstash-mind`, `recallable`. Decide before the repo gets a remote.
2. **License.** vstash is presumably MIT/Apache — match it unless there's a reason not to.
3. **Repo location.** Sibling of vstash on GitHub (`stffns/engram`) vs. monorepo. Sibling is the default; only consider monorepo if there's a clear coupling reason.
...
3. **Bootstrap the repo:**
   ...
   - `engram/__init__.py` with `__version__ = "0.1.0"` and `from .memory import Memory`.
   - `engram/memory.py` with a `Memory` class that wraps `vstash.Memory` and exposes a single method: `remember(text: str, layer: str = "episodic")`. That's it. No consolidation yet, no decision policies. Just the wiring.
...
> **Project:** engram — agent-loop layer on top of vstash.
