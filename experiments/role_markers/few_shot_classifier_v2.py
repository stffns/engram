"""Few-shot semantic role classifier V2 (#39b Phase 1).

V2 differs from V1 only in the role taxonomy: DECISION is replaced by
STATE_CHANGE_REPORT, with non-overlapping lexical markers vs PREFERENCE
(past tense + concrete state-change verbs + specific entity, no
comparative "X over Y" structure). Other 3 roles unchanged. See
prototype_style_guide.md V2 section for definitions; notes/role_markers_39.md
for the V1 finding that motivates this redo.

V1 result: macro-F1 0.753 +- 0.047 across 3 seeds; DECISION recall
38% (collides with PREFERENCE); other 3 roles 87-95% recall.

V2 hypothesis: if STATE_CHANGE_REPORT separates from PREFERENCE
at >= 0.85 recall while keeping the other 3 roles >= 0.85, the V1
collision was a taxonomy artifact, not a BGE limitation.

Loads prototypes_v2.json + eval_v2.json, embeds via vstash.embed.embed_texts,
...
(unchanged from V1):
    ...
    macro-F1 >= 0.85 AND per-class >= 0.75 -> STRONG
...
DEFAULT_PROTOTYPES = Path("experiments/role_markers/prototypes_v2.json")
DEFAULT_EVAL = Path("experiments/role_markers/eval_v2.json")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.output_dir is None:
        ...
        args.output_dir = Path("experiments/role_markers/runs") / f"phase1_v2_{ts}"
