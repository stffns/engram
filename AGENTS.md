...
This file is the entry point for any Codex session opened in the
merken repo. It should be cheap to read and keep a current Codex
...
1. [`CONSTITUTION.md`](CONSTITUTION.md) — what merken is, what it
   ...
   *before* any code.
2. The vstash repo's `AGENTS.md` — merken is a strict consumer of
...
**Hook bug fixed (2026-04-17):** `~/.Codex/hooks/merken-save.sh`
...
- **MCP server** — `merken-mcp` on `$PATH`, `python -m merken.mcp_server`,
  or `Codex mcp add merken -- python -m merken.mcp_server`. Eight
...
- **Codex hooks** — live in `~/.Codex/settings.json`.
  ...
  Scripts at `~/.Codex/hooks/merken-*.sh`.
...
- **brief_v1 integration into Codex hooks.** Generate briefs
