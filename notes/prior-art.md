...
> A single place where merken tracks notes on existing memory systems
...
problem merken targets — an agent-loop layer over a retrieval substrate.
...
- **Raw verbatim storage beats LLM-extracted facts on retrieval
  ...
  merken:** consolidation episodic→semantic is *additive*, not
...
- **Metadata filtering is the largest single retrieval lever.** Their
  ...
  `(wing, room)`. They admit it. **Implication for merken:** vstash
...
- **MCP is the integration surface that matters.** Adoption story is
  ...
  for merken:** ship the SDK first to keep the loop honest, but don't
  defer MCP to v0.5 either.
- **Auto-save hooks change the user contract.** Without write triggers,
  no memory accumulates. **Implication:** merken needs a Claude Code
...
- **Temporal knowledge graphs in SQLite are tractable.**
  ...
  file, no Neo4j. **Implication:** if merken ever wants a KG, it
...
- **AAAK lossy compression was over-claimed.** "30x lossless" turned
  ...
  **Lesson for merken:** no bespoke compression dialect. If we ever
...
- **`store everything` as the only policy.** It wins LongMemEval but
  ...
  is the whole reason merken exists.

### How this maps to merken decisions

| mempalace finding | merken decision |
|---|---|
...
| 19 MCP tools is a smell | Cap merken MCP at ~6 |
...
## neo4j-labs/agent-memory — merken's cousin, not its competitor

> *Analyzed 2026-04-10, after merken's four decision primitives were
> complete. neo4j-labs/agent-memory is the closest published system
> to merken in **shape** — both are policy/loop layers sitting on
...
│            │ FTS5 + RRF)                      │ search)                          │
├────────────┼──────────────────────────────────┼──────────────────────────────────┤
│ Agent-loop │ merken (4 decision primitives    │ neo4j-labs/agent-memory          │
...
- merken: `should_remember` / `should_recall` /
...
**Dense substrate + policy loop (merken/vstash):**
...
- Read cost: `vstash.search` + interleave + the *caller's* LLM
  reasoning over the hits. merken itself does not reason — it
  returns strings.
...
```
                     merken/vstash                neo4j-labs/neo4j
...
The asymmetry that matters most: **merken never loses
...
### merken already has a seed of graph structure

Every semantic `Fact` in merken has `derived_from: list[str]`
...
A future `merken_edges` table would generalize this implicit
...
1. **Extraction source is the unsolved problem.** LLM
   ...
   pipeline dependency merken has explicitly avoided. User-
...
1. **merken and neo4j-labs are parallel, not competing.**
   Comparing them on LongMemEval (dense retrieval) would be
   like comparing a car and a boat on a road — the graph
   system isn't built for that benchmark and merken isn't
   built for entity-centric queries.

2. **The right framing for merken in the landscape:**
   merken is the **policy-loop layer for dense-retrieval
...
3. **If merken ever adds a graph view,** the honest path is
...
| neo4j-labs finding | merken decision |
|---|---|
...
| Dense-retrieval vs symbolic-extraction are two memory theories | merken explicitly bets on the dense side. The bet is: embeddings are sufficient and the loop only needs to decide what to keep/promote/forget. Reviewable if a scenario shows otherwise. |
